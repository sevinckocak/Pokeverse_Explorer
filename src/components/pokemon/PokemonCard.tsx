...
import { getPokemonSpriteUrl } from "@/utils/pokemonAssets";
...
// List items only carry `{ id, name }` (see PokemonListItem) — no type
...
function PokemonCardComponent({ pokemon, width, onPress }: PokemonCardProps) {
  ...
  // `pokemon.id` already comes fully resolved from the domain model — no
  // component parses a PokeAPI url to get here.
  const displayId =
    pokemon.id !== null
      ? `#${pokemon.id.toString().padStart(3, "0")}`
      : ID_PLACEHOLDER;
  const imageUrl = pokemon.id !== null ? getPokemonSpriteUrl(pokemon.id) : null;
